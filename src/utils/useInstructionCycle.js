...
const useInstructionCycle = (systemState, setSystemState, handleOutput) => {
  ...
  const executeALUOperation = useCallback((operation, operandA, operandB = '0'.repeat(WORD_SIZE)) => {
    try {
      // Normalizar operandos
      const opA = normalizeBinary(operandA, WORD_SIZE, true);
      const opB = operation !== 'NOT' && operation !== 'SHL' && operation !== 'SHR' 
      ...
      if (operation === 'NOT' || operation === 'SHL' || operation === 'SHR') {
        ...
        result = {
          ...
          flags: aluResult.flags || {
            ...
            C: aluResult.carry || '0',
            S: (operation === 'SHR') ? '0' : (aluResult.result || aluResult)[0] === '1' ? '1' : '0',
            O: '0'
          }
        };
      } else {
  ...
  const executeNextInstruction = useCallback(() => {
    try {
      ...
      const instruction = normalizeBinary(systemState.memory.rom[pcNum], INSTRUCTION_SIZE);
      const { opcode, operand } = decodeInstruction(instruction);
      const address = normalizeBinary(operand, ADDRESS_SIZE);

      handleOutput('text', `[PC: ${pc}] Ejecutando: ${INSTRUCTION_SET[opcode]?.name || 'DESCONOCIDA'} ${address}`);
      ...
      switch(newState.currentStep) {
        ...
        case CPU_STEPS.EXECUTE:
          ...
          newState.cpu.alu.operandA = normalizeBinary(newState.cpu.registers.ACC, WORD_SIZE, true);
          ...
          switch(opcode) {
            ...
            {
              const memAddress = fromBinary(address);
              console.log('STORE ejecutado: ACC =', newState.cpu.registers.ACC, '-> RAM[', memAddress, ']');
              newState.memory.ram[memAddress] = newState.cpu.registers.ACC.padStart(WORD_SIZE, '0');
              // Actualiza MAR y MBR para visualización
              newState.cpu.registers.MAR = address;
              newState.cpu.registers.MBR = newState.cpu.registers.ACC.padStart(WORD_SIZE, '0');
          ...
          if(incrementarPC) {
            ...
            newState.cpu.registers.PC = normalizeBinary(toBinary(nextPC, ADDRESS_SIZE), ADDRESS_SIZE);
          }
